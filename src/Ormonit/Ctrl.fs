...
let rec closeTimedoutSrvs (timeoutMark:DateTimeOffset) (service:OpenServiceData) (execc:Execc) =
    ...
    | srv ->
        ...
        | false, _ -> ()
        let lid = srv.logicId
        let args = sprintf "--logic-id %d --public-key %s --ctrl-address %s --open-service %s" lid execc.publicKey ca srv.fileName
...
let rec supervise (sok) (nsok) (msgs:Comm.TMsg list) (execc:Execc) : unit =
    ...
    | "close" ->
        ...
        | Comm.Msg (ckey, note) ->
            ...
            | None ->
                log(warnl(sprintf """Unable to find ckey "%s". Aknowledgement %s.""" ckey exitCode))
            | Some lid ->
    ...
    | "sys:init-service" ->
        ...
        | Comm.Msg (lid, n) ->
            log(tracel(sprintf """Sent "client-key" note."""))
        | Comm.Error (errn, errm) ->
            log(errorl(sprintf """Error %i on "client-key" (send). %s.""" errn errm))
        match Comm.recv sok with
        | Comm.Error (errn, errm) ->
        ...
        | Comm.Msg (logicId, ekey) ->
            ...
            let vckey = 
                ...
                | None -> false
                | Some srv ->
                    not (execc.lids.ContainsKey clientKey) && String.IsNullOrEmpty(srv.key)
    ...
    | _ ->
        ...
        | _ ->
            ...
            | Comm.Msg (ckey, status) ->
                match execc.lids.TryFind ckey with
                | None ->
                    log(warnl(sprintf """Reply "report-status" from unknown service [%s] status code %s.""" ckey status))
                | Some lid ->
...
let openMaster (execc:Execc) =
    ...
    |> List.iteri (fun i it ->
        let ckey = randomKey ()
        let lid =  index + i
        let args = sprintf "--logic-id %d --public-key %s --ctrl-address %s --open-service %s" lid execc.publicKey ca it.Location
