...
namespace Cilnn
{
    ...
    public static class NanomsgLibraryLoader
    {
        ...
        static string calculatexdir(string assemblyPath, string framework, string libFile)
        {
            ...
            return Path.Combine(dir, "content", framework, RuntimeInformation.ProcessArchitecture == Architecture.X64 ? "x64" : "x86", libFile);
        }
            ...
            var paths = new[]
                {
                    ...
                    Path.Combine(assemblyDirectory, "bin", RuntimeInformation.ProcessArchitecture == Architecture.X64 ? "x64" : "x86", libFile),
                    Path.Combine(assemblyDirectory, RuntimeInformation.ProcessArchitecture == Architecture.X64 ? "x64" : "x86", libFile),
                    Path.Combine(assemblyDirectory, libFile),

                    Path.Combine(rootDirectory, "bin", RuntimeInformation.ProcessArchitecture == Architecture.X64 ? "x64" : "x86", libFile),
                    Path.Combine(rootDirectory, RuntimeInformation.ProcessArchitecture == Architecture.X64 ? "x64" : "x86", libFile),
